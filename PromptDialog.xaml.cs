...
{
    PromptDialog(Window owner, string title, string label, string text, string okText)
    {
        InitializeComponent();
        Owner = owner;
        Title = title;
        ...
        Loaded += (_, _) =>
        {
            ...
            Input.Select(0, dot > 0 ? dot : text.Length);
        };
    }

    /// The text the user entered, or null if cancelled or left empty.
    public static string Ask(Window owner, string title, string label, string text, string okText)
    {
        var dialog = new PromptDialog(owner, title, label, text, okText);
